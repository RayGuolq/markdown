...
# markdown使用方法
如果在观看时有什么问题，可以直接查阅源文件。另外需要说明的是GitHub对Markdown的支持增加了一些扩展功能，因此在GitHub上可以渲染的Markdown语法并不一定在仅支持标准Markdown语法的平台上运行。

**注意：在编写markdown文件时，所有的新格式的开始和结束，最好都空一行出来，避免语法失效**
...
## 有序列表
- 在文字的开头使用`数字+.`*(数字不在乎是否有序)*，与文字直接还需加一个`空格`。
- 子序列需要在父序列的基础上向左缩进填补`空格`到父序列的第一个文字或字母。
...
## 任务列表
任务列表是GitHub对标准Markdown语法的扩展，并不是标准Markdown语法。因此MarkdownPad不会对该语法进行渲染。但是使用Atom就会对该语法进行渲染。
...
## 使用表情
GitHub中的Markdown语法扩展中包括了一些表情包。这个表情包语法依然可以通过Atom进行渲染。由于不是标准的Markdown语法不能通过MarkdownPad渲染。
